  ...
  return (
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="text-xs font-bold uppercase tracking-[.16em] text-orange-600">Concierge pipeline</p><h1 className="mt-1 text-3xl font-black text-slate-950">Custom Trip Requests</h1><p className="mt-2 text-sm text-slate-500">Review traveler preferences, call the booker and move each plan forward.</p></div>
      ...
      <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {statuses.map((status) => <button key={status} onClick={() => setFilter(status)} className={`rounded-2xl border bg-white p-4 text-left transition ${filter === status ? "border-orange-300 shadow-md ring-2 ring-orange-100" : "border-slate-100 hover:border-slate-200"}`}><p className="text-xs font-bold capitalize text-slate-500">{status}</p><p className="mt-2 text-3xl font-black text-slate-950">{count(status)}</p></button>)}
      </div>

      <div className="mt-6 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row">
        <label className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} className="w-full rounded-xl border border-slate-200 py-2.5 pl-10 pr-4 text-sm outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-100" placeholder="Search reference, ticket, name, phone or destination" /></label>
      ...
      {loading ? <div className="grid h-64 place-items-center"><Loader2 className="h-9 w-9 animate-spin text-orange-500" /></div> : error ? <div className="mt-6 rounded-2xl border border-rose-200 bg-rose-50 p-5 text-sm font-semibold text-rose-700">{error}</div> : filtered.length === 0 ? <div className="mt-6 rounded-3xl border border-dashed border-slate-300 bg-white p-14 text-center"><Sparkles className="mx-auto h-9 w-9 text-slate-300" /><p className="mt-4 font-bold text-slate-700">No matching custom trip requests</p></div> : (
      ...
      {selected && <><button aria-label="Close request details" onClick={() => setSelected(null)} className="fixed inset-0 z-[100] bg-slate-950/55 backdrop-blur-sm" /><aside className="fixed inset-y-0 right-0 z-[101] w-full max-w-xl overflow-y-auto bg-white shadow-2xl"><div className="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white/95 px-6 py-5 backdrop-blur"><div><p className="text-xs font-bold uppercase tracking-wider text-orange-600">Custom trip consultation</p><h2 className="mt-1 text-xl font-black">{selected.booker_name}</h2></div><button onClick={() => setSelected(null)} className="grid h-10 w-10 place-items-center rounded-full bg-slate-100 hover:bg-slate-200"><X className="h-5 w-5" /></button></div>
        <div className="space-y-6 p-6">
          <div className="flex items-center justify-between rounded-xl border border-orange-200 bg-orange-50 px-4 py-3"><span className="text-xs font-semibold text-orange-800">Customer reference</span><span className="font-mono text-sm font-black tracking-wider text-orange-700">#{requestReference(selected.id)}</span></div>
          <div className="grid gap-3 sm:grid-cols-2"><a href={`tel:${selected.phone}`} className="rounded-2xl bg-slate-950 p-5 text-white hover:bg-orange-600"><Phone className="h-5 w-5 text-orange-300" /><p className="mt-4 text-xs text-slate-400">Call booker</p><p className="mt-1 font-black">{selected.phone}</p></a><div className="rounded-2xl bg-orange-50 p-5"><MapPin className="h-5 w-5 text-orange-600" /><p className="mt-4 text-xs text-orange-700">Preferred trip</p><p className="mt-1 font-black capitalize text-slate-900">{requestDestination(selected)}</p></div></div>

...
export default function CustomTripForm() {
  ...
  if (requestId) {
    return (
      <section className="relative flex min-h-[82vh] items-center overflow-hidden px-5 pb-20 pt-32 sm:px-8">
        ...
        <div className="relative mx-auto max-w-2xl rounded-[2.5rem] border border-white bg-white/90 p-8 text-center shadow-[0_30px_100px_-30px_rgba(15,23,42,.28)] backdrop-blur sm:p-14">
          ...
          <p className="mt-7 text-xs font-black uppercase tracking-[.2em] text-orange-600">Request received</p>
          <h1 className="mt-3 text-4xl font-black tracking-tight sm:text-5xl">Your dream trip starts here.</h1>
  ...
  return (
    <>
      <section className="relative overflow-hidden border-b border-slate-200 bg-[#f7f8fa] px-5 pb-28 pt-32 sm:px-8 lg:pb-36 lg:pt-40">
        <div className="absolute -left-40 top-16 h-96 w-96 rounded-full bg-orange-100/80 blur-3xl" />
        <div className="absolute right-0 top-0 h-full w-1/2 bg-[linear-gradient(135deg,transparent_20%,rgba(255,237,213,.65)_100%)]" />
        <div className="relative mx-auto grid max-w-7xl items-center gap-14 lg:grid-cols-[minmax(0,.9fr)_minmax(520px,1.1fr)]">
          <div className="max-w-2xl">
            <span className="inline-flex items-center gap-2 rounded-full border border-orange-200 bg-white px-4 py-2 text-xs font-extrabold uppercase tracking-[.14em] text-orange-600 shadow-sm"><Sparkles className="h-4 w-4" />Personalized group travel</span>
            <h1 className="mt-7 text-balance text-5xl font-extrabold leading-[1.08] tracking-[-.045em] text-slate-950 sm:text-6xl lg:text-[4.35rem]">A trip designed around the people you love.</h1>
      ...
      <section className="relative z-10 mx-auto -mt-16 max-w-7xl px-5 pb-24 sm:px-8">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px]">
          <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_22px_60px_-32px_rgba(15,23,42,.35)] sm:p-9 lg:p-11">
            <div className="flex items-start gap-4 border-b border-slate-100 pb-6"><span className="mt-0.5 grid h-8 w-8 place-items-center rounded-full bg-orange-50 text-xs font-extrabold text-orange-600 ring-1 ring-orange-200">1</span><div><p className="text-xs font-bold uppercase tracking-[.12em] text-slate-400">Contact details</p><h2 className="mt-1 text-2xl font-bold tracking-tight">Who should we speak with?</h2></div></div>

            <div className="mt-8 grid gap-5 sm:grid-cols-2">
              <label className="text-sm font-bold text-slate-700">Booker name<span className="text-orange-500"> *</span><input required maxLength={120} value={form.bookerName} onChange={(e) => updateForm("bookerName", e.target.value)} className="premium-input mt-2" placeholder="Your full name" /></label>
              <label className="text-sm font-bold text-slate-700">Email address<span className="text-orange-500"> *</span><input required type="email" inputMode="email" autoComplete="email" maxLength={254} value={form.email} onChange={(e) => updateForm("email", e.target.value)} className="premium-input mt-2" placeholder="Enter your email address" /></label>
              <label className="text-sm font-bold text-slate-700 sm:col-span-2">Active phone number<span className="text-orange-500"> *</span><input required type="tel" inputMode="tel" autoComplete="tel" maxLength={30} value={form.phone} onChange={(e) => updateForm("phone", e.target.value)} className="premium-input mt-2" placeholder="Enter your active phone number" /><span className="mt-2 block text-xs font-normal leading-5 text-slate-500">Enter a real number you personally use. Please do not enter a random or temporary number—our travel specialist will call you here.</span></label>
            </div>

            <div className="mt-5 rounded-xl border border-emerald-200 bg-emerald-50/70 p-4">
              <label className="flex cursor-pointer items-start gap-3"><input type="checkbox" checked={phoneIsWhatsapp} onChange={(event) => setPhoneIsWhatsapp(event.target.checked)} className="mt-0.5 h-4 w-4 rounded border-emerald-300 accent-emerald-600" /><span><span className="flex items-center gap-2 text-sm font-bold text-emerald-900"><MessageCircle className="h-4 w-4" />This phone number is also on WhatsApp</span><span className="mt-1 block text-xs leading-5 text-emerald-800/75">WhatsApp helps our team share itinerary options and trip details conveniently.</span></span></label>
              {!phoneIsWhatsapp && <label className="mt-4 block border-t border-emerald-200 pt-4 text-sm font-bold text-slate-700">WhatsApp number<span className="text-orange-500"> *</span><input required type="tel" inputMode="tel" autoComplete="tel" maxLength={30} value={form.whatsappNumber} onChange={(e) => updateForm("whatsappNumber", e.target.value)} className="premium-input mt-2" placeholder="Enter your WhatsApp number" /><span className="mt-2 block text-xs font-normal leading-5 text-slate-500">Include the country code if this WhatsApp number is different from your calling number.</span></label>}
            </div>

            <div className="mt-12 flex items-start gap-4 border-b border-slate-100 pb-6"><span className="mt-0.5 grid h-8 w-8 place-items-center rounded-full bg-orange-50 text-xs font-extrabold text-orange-600 ring-1 ring-orange-200">2</span><div><p className="text-xs font-bold uppercase tracking-[.12em] text-slate-400">Your group · minimum two</p><h2 className="mt-1 text-2xl font-bold tracking-tight">Who is traveling?</h2></div></div>
            <div className="mt-7 space-y-3">
              {travelers.map((traveler, index) => <div key={index} className="flex items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-slate-100 text-sm font-black text-slate-500">{index + 1}</span><input required maxLength={120} value={traveler} onChange={(e) => updateTraveler(index, e.target.value)} className="premium-input" placeholder={index === 0 ? "Lead traveler name" : "Traveler name"} aria-label={`Traveler ${index + 1} name`} />{travelers.length > 2 && <button type="button" onClick={() => setTravelers((current) => current.filter((_, itemIndex) => itemIndex !== index))} className="grid h-11 w-11 shrink-0 place-items-center rounded-xl text-rose-500 hover:bg-rose-50" aria-label={`Remove traveler ${index + 1}`}><Trash2 className="h-4 w-4" /></button>}</div>)}
            ...
            <button disabled={submitting || (!destinationKnown && !form.placeType)} className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 px-6 py-4 text-base font-black text-white shadow-xl shadow-slate-900/15 transition hover:-translate-y-0.5 hover:bg-orange-600 disabled:cursor-not-allowed disabled:opacity-50">{submitting ? <><Loader2 className="h-5 w-5 animate-spin" />Sending request...</> : <>Book my free consultation <ArrowRight className="h-5 w-5" /></>}</button>

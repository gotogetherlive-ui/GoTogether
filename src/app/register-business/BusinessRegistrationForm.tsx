...
export default function BusinessRegistrationForm({ introduction }: { introduction: BusinessIntroduction | null }) {
  ...
  const [form, setForm] = useState({
    company_name: introduction?.travel_name || "",
    location: "",
    country_code: "+91",
    phone_number: introduction?.phone_number.startsWith('+91') ? introduction.phone_number.slice(3).replace(/\D/g, '') : "",
    alternate_email: "",
    profile_pic_url: "",
    pan_number: "",
    pan_photo_url: "",
  ...
  if (profileComplete === false) {
    return (
      <div className="min-h-[80vh] flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-xl border border-slate-100 shadow-xl p-8 text-center">
          ...
          <button
            ...
            className="w-full bg-slate-900 text-white font-bold py-3.5 rounded-2xl shadow-lg hover:shadow-xl hover:shadow-orange-500/20 transition-all flex items-center justify-center gap-2"
          >
  ...
  if (applicationStatus === "pending") {
    return (
      <div className="min-h-[85vh] flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-xl border border-slate-100 shadow-xl p-8 text-center relative overflow-hidden">
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 py-12 px-4 flex justify-center items-center">
      <div className="max-w-2xl w-full">
        {introduction?.status === 'approved' && <RegistrationProgress status="approved" />}
        {/* Header */}
        <div className="text-center mb-8">
          <h1 className="gt-page-title text-3xl font-bold text-slate-900 mb-2 tracking-tight">Register Your Business</h1>
        ...
        <div className="bg-white rounded-xl border border-slate-100 shadow-xl p-8 relative overflow-hidden">
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col items-center">
                <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                  ...
                  <input aria-label="Upload company logo" name="company-logo" ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleFileChange} />
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input aria-label={'Company name'} name={'company-name'} type="text" placeholder="Enter registered company name" value={form.company_name} onChange={(e) => setForm({ ...form, company_name: e.target.value })} className="premium-input" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Base Location *</label>
                  <input aria-label={'Location'} name={'location'} type="text" placeholder="Mumbai, India" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} className="premium-input" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input aria-label={'Phone number'} name={'phone-number'} type="text" placeholder="Enter mobile number" value={form.phone_number} onChange={(e) => setForm({ ...form, phone_number: e.target.value.replace(/\D/g, '') })} className="premium-input" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Alternate Email</label>
                  <input aria-label={'Alternate email'} name={'alternate-email'} type="email" placeholder="Enter alternate email address" value={form.alternate_email} onChange={(e) => setForm({ ...form, alternate_email: e.target.value })} className="premium-input" />
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">PAN Card Number *</label>
                  <input aria-label={'Pan number'} name={'pan-number'} type="text" placeholder="ABCDE1234F" maxLength={10} value={form.pan_number} onChange={(e) => setForm({ ...form, pan_number: e.target.value.toUpperCase() })} className="premium-input" />
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">PAN Photo (Under 5MB) *</label>
                  ...
                  <input aria-label="Upload PAN card photo" name="pan-card-photo" ref={panInputRef} type="file" accept="image/*" className="hidden" onChange={handlePanPhotoChange} />
                  {panPhotoError && (
                ...
                {[
                  ...
                  <label key={prov.id} className={`p-4 rounded-2xl border transition-all cursor-pointer flex flex-col justify-between ${
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <input name="payment-provider" type="radio" checked={form.payment_provider === prov.id} onChange={() => setForm({ ...form, payment_provider: prov.id })} className="text-orange-500" />
                    </div>
          ...
          {step === 3 && (
            <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">{providerCopy.apiKeyLabel} *</label>
                  <input aria-label={'Api key'} name={'api-key'} type="text" placeholder={providerCopy.apiKeyPlaceholder} value={form.api_key} onChange={(e) => setForm({ ...form, api_key: e.target.value })} className="premium-input" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">{providerCopy.apiSecretLabel} *</label>
                  <input aria-label={'Api secret'} name={'api-secret'} type="password" placeholder={providerCopy.apiSecretPlaceholder} value={form.api_secret} onChange={(e) => setForm({ ...form, api_secret: e.target.value })} className="premium-input" />
                </div>
              </div>
              ...
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-4">
                ...
                <div className="text-xs text-slate-600 space-y-2 leading-relaxed font-medium">
                  <div className="font-bold text-slate-800 uppercase tracking-wider">Instructions:</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {requiresWebhookSecret && (
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">{providerCopy.webhookSecretLabel} *</label>
                    <input aria-label={'Webhook secret'} name={'webhook-secret'} type="password" placeholder={providerCopy.webhookSecretPlaceholder} value={form.webhook_secret} onChange={(e) => setForm({ ...form, webhook_secret: e.target.value })} className="premium-input" />
                  </div>
                )}
                <div className={requiresWebhookSecret ? "space-y-1" : "space-y-1 md:col-span-2"}>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">{providerCopy.accountLabel} *</label>
                  <input aria-label={'Razorpay account id'} name={'razorpay-account-id'} type="text" placeholder={providerCopy.accountPlaceholder} value={form.razorpay_account_id} onChange={(e) => setForm({ ...form, razorpay_account_id: e.target.value })} className="premium-input" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input aria-label={'Razorpay account holder name'} name={'razorpay-account-holder-name'} type="text" placeholder="As shown on gateway profile" value={form.razorpay_account_holder_name} onChange={(e) => setForm({ ...form, razorpay_account_holder_name: e.target.value })} className="premium-input" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Registered Email *</label>
                  <input aria-label={'Razorpay account email'} name={'razorpay-account-email'} type="email" placeholder="Enter gateway-registered email" value={form.razorpay_account_email} onChange={(e) => setForm({ ...form, razorpay_account_email: e.target.value })} className="premium-input" />
                </div>
              </div>

              <div className="space-y-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Registered Phone Number *</label>
                  <input aria-label={'Razorpay account phone'} name={'razorpay-account-phone'} type="tel" placeholder="Gateway registered mobile" value={form.razorpay_account_phone} onChange={(e) => setForm({ ...form, razorpay_account_phone: e.target.value })} className="premium-input" />
                </div>
                ...
                <label className="flex items-start gap-3 rounded-2xl border border-orange-100 bg-orange-50/60 p-4 text-xs text-slate-700 cursor-pointer">
                  <input name={'payment-terms-accepted'} type="checkbox" checked={form.payment_terms_accepted} onChange={(e) => setForm({ ...form, payment_terms_accepted: e.target.checked })} className="mt-0.5" />
            ...
            <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 overflow-hidden">
                <div className="border-b border-slate-200 bg-white px-5 py-4">
                  <p className="font-bold text-slate-900">{ORGANIZER_AGREEMENT_TITLE}</p>
              ...
              <div className="rounded-2xl border border-orange-100 bg-orange-50/50 p-5 space-y-4">
                <div>
                  <label htmlFor="organizer-signature" className="text-xs font-bold text-slate-600 uppercase tracking-wider">
              ...
              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 text-center space-y-4">
                {verifying ? (
                  ...
                  <div className="py-8 flex flex-col items-center gap-3">
                    ...
                    <span className="text-base font-bold text-slate-800">Verification Succeeded!</span>
                    <p className="text-xs text-slate-500 max-w-xs">{verificationResult.message}</p>
                  </div>
                ) : (
                  <div className="py-8 flex flex-col items-center gap-3">
                    <XCircle className="w-12 h-12 text-rose-500" />
                    <span className="text-base font-bold text-slate-800">Verification Failed</span>
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 flex justify-between items-center gap-4">
            ...
            {step < 5 ? (
              ...
              <button
                ...
                className="px-6 py-3 rounded-2xl bg-slate-900 hover:bg-slate-700 text-white text-sm font-bold flex items-center gap-2 shadow-lg shadow-orange-500/20 disabled:opacity-50 transition-all"
              >

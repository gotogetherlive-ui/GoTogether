    ...
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <main>
        <section className="relative min-h-[680px] overflow-hidden bg-slate-950 pt-24 text-white">
          ...
          <div className="relative mx-auto flex min-h-[590px] max-w-7xl items-end px-5 pb-16 sm:px-8 lg:pb-20">
            <div className="max-w-4xl">
              ...
              <h1 className="max-w-4xl text-balance text-5xl font-black leading-[1.02] tracking-[-.045em] sm:text-6xl lg:text-7xl">{guide.title}</h1>
        ...
        <section className="relative z-10 mx-auto -mt-7 max-w-7xl px-5 sm:px-8">
          <dl className="grid overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_25px_80px_-35px_rgba(15,23,42,.35)] md:grid-cols-3">
            {[
          ...
          <div className="grid gap-14 lg:grid-cols-[minmax(0,1fr)_320px]">
            <article className="min-w-0">
              ...
              <div className="mt-16 space-y-16">
                {allSections.map((section, index) => <section key={section.title} id={`section-${index + 1}`}><div className="flex items-start gap-4"><span className="mt-1 text-sm font-black text-orange-500">{String(index + 1).padStart(2, "0")}</span><div><h2 className="text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">{section.title}</h2><div className="mt-5 space-y-5 text-[17px] leading-8 text-slate-600">{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></div></div></section>)}
              </div>
            </article>

            <aside className="self-start lg:sticky lg:top-28">
              <div className="rounded-3xl bg-slate-950 p-6 text-white"><Compass className="h-7 w-7 text-orange-400" /><h2 className="mt-4 text-xl font-black">Plan your trip</h2><nav className="mt-5 space-y-1" aria-label="Guide contents">{allSections.map((section, index) => <a key={section.title} href={`#section-${index + 1}`} className="flex items-center justify-between rounded-xl px-3 py-2.5 text-sm font-semibold text-slate-300 hover:bg-white/10 hover:text-white"><span>{section.title}</span><ArrowRight className="h-4 w-4 shrink-0" /></a>)}</nav><a href="#available-trips" className="mt-6 inline-flex w-full items-center justify-center rounded-xl bg-orange-500 px-4 py-3 text-sm font-bold hover:bg-orange-400">Compare departures</a></div>
            </aside>
          </div>
        </section>
        ...
        <section className="bg-white py-20 lg:py-24">
          <div className="mx-auto max-w-7xl px-5 sm:px-8">
            <p className="text-xs font-bold uppercase tracking-[.18em] text-orange-600">Worth adding</p><h2 className="mt-2 max-w-3xl text-4xl font-black tracking-tight text-slate-950">Nearby attractions and route ideas</h2>
            <div className="mt-9 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{guide.attractions.map((item) => <article key={item.name} className="rounded-3xl border border-slate-200 bg-slate-50 p-6"><MapPin className="h-6 w-6 text-orange-500" /><h3 className="mt-4 text-xl font-black">{item.name}</h3><p className="mt-2 leading-7 text-slate-600">{item.description}</p><p className="mt-4 border-l-2 border-orange-300 pl-4 text-sm font-medium leading-6 text-slate-500">{item.travelNote}</p></article>)}</div>
            <div className="mt-10 grid gap-4 md:grid-cols-3">{guide.gallery.map((image) => <div key={image.src + image.alt} className="relative aspect-[4/3] overflow-hidden rounded-3xl"><Image src={image.src} alt={image.alt} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover" /></div>)}</div>
          </div>
        </section>

        <section id="available-trips" className="mx-auto max-w-7xl scroll-mt-24 px-5 py-20 sm:px-8 lg:py-28">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-orange-600">Live marketplace inventory</p><h2 className="mt-2 text-4xl font-black tracking-tight text-slate-950">Available trips</h2><p className="mt-3 max-w-2xl leading-7 text-slate-600">These departures come from current public listings by GoTogether organizers. Dates and seats can change, so open a trip to verify the latest details.</p></div><Link href="/trips" className="inline-flex items-center gap-2 font-bold text-orange-600">Browse all trips <ArrowRight className="h-4 w-4" /></Link></div>
          {trips.length > 0 ? <div className="mt-10 grid gap-7 md:grid-cols-2 xl:grid-cols-3">{trips.map((trip) => <TripCard key={trip.id} trip={trip} />)}</div> : <div className="mt-10 rounded-3xl border border-dashed border-orange-200 bg-orange-50 p-8 sm:p-10"><ShieldCheck className="h-8 w-8 text-orange-500" /><h3 className="mt-4 text-2xl font-black">No matching departure is live right now</h3><p className="mt-2 max-w-2xl leading-7 text-slate-600">Organizer inventory changes throughout the season. Explore all live trips or return later for a departure matching this guide.</p><Link href="/trips" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-bold text-white">Explore all trips <ArrowRight className="h-4 w-4" /></Link></div>}
        </section>

        <section className="bg-white py-20 lg:py-24"><div className="mx-auto max-w-5xl px-5 sm:px-8"><p className="text-xs font-bold uppercase tracking-[.18em] text-orange-600">Questions answered</p><h2 className="mt-2 text-4xl font-black tracking-tight text-slate-950">Frequently asked questions</h2><div className="mt-8 divide-y divide-slate-100 overflow-hidden rounded-3xl border border-slate-200">{guide.faqs.map((faq) => <details key={faq.question} className="group px-5 sm:px-7"><summary className="flex cursor-pointer list-none items-center justify-between gap-5 py-6 font-bold [&::-webkit-details-marker]:hidden">{faq.question}<ChevronDown className="h-5 w-5 shrink-0 text-slate-400 transition group-open:rotate-180 group-open:text-orange-600" /></summary><p className="max-w-3xl pb-6 pr-8 leading-7 text-slate-600">{faq.answer}</p></details>)}</div></div></section>
        <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8"><h2 className="text-3xl font-black tracking-tight">Continue planning</h2><div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{guide.related.map((item) => <Link key={item.href} href={item.href} className="group flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-5 font-bold hover:border-orange-200 hover:text-orange-600">{item.label}<ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></Link>)}</div></section>
      </main>
      <Footer />
    </div>
  );

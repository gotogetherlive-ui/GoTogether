...
function ShareAvatar({ story }: { story: ShareableStory }) {
  ...
  return (
    <div className="relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-gradient-to-br from-orange-100 to-rose-100 text-sm font-extrabold text-slate-700">
...
export default function StoryShareSheet({ story, copied, shareUrl, onClose, onCopy, onOpenTarget }: Props) {
  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm animate-in fade-in duration-200 md:items-center md:p-4">
      <div className="w-full overflow-hidden rounded-t-[2rem] border border-white/80 bg-white shadow-2xl animate-in slide-in-from-bottom-6 duration-200 md:max-w-md md:rounded-[2rem] md:zoom-in-95">
        <div className="relative h-32 bg-gradient-to-tr from-slate-950 via-slate-900 to-indigo-900">
          ...
          <div className="absolute bottom-4 left-5 text-white">
            <p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-orange-300">Share story</p>
            <h3 className="text-xl font-extrabold leading-tight">Send this travel moment</h3>
        ...
        <div className="space-y-5 p-5">
          <div className="flex items-start gap-3 rounded-2xl border border-slate-100 bg-slate-50 p-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <p className="truncate text-sm font-extrabold text-slate-800">{story.author_name}</p>
          ...
          <div className="grid grid-cols-4 gap-3">
            <button onClick={() => onOpenTarget("whatsapp")} className="group flex flex-col items-center gap-2 rounded-2xl border border-emerald-100 bg-emerald-50 p-3 text-emerald-700 transition-colors hover:bg-emerald-100">
              <span className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-500 text-white transition-transform group-hover:scale-105"><MessageCircle className="h-4 w-4" /></span><span className="text-[10px] font-bold">WhatsApp</span>
            </button>
            <button onClick={() => onOpenTarget("x")} className="group flex flex-col items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-3 text-slate-800 transition-colors hover:bg-slate-100">
              <span className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-950 text-sm font-extrabold text-white transition-transform group-hover:scale-105">X</span><span className="text-[10px] font-bold">X</span>
            </button>
            <button onClick={() => onOpenTarget("facebook")} className="group flex flex-col items-center gap-2 rounded-2xl border border-blue-100 bg-blue-50 p-3 text-blue-700 transition-colors hover:bg-blue-100">
              <span className="flex h-10 w-10 items-center justify-center rounded-full bg-blue-600 text-sm font-extrabold text-white transition-transform group-hover:scale-105">f</span><span className="text-[10px] font-bold">Facebook</span>
            </button>
            <button onClick={onCopy} className="group flex flex-col items-center gap-2 rounded-2xl border border-violet-100 bg-violet-50 p-3 text-violet-700 transition-colors hover:bg-violet-100">
              <span className="flex h-10 w-10 items-center justify-center rounded-full bg-violet-600 text-white transition-transform group-hover:scale-105">{copied ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}</span><span className="text-[10px] font-bold">{copied ? "Copied" : "Copy link"}</span>
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2">
            ...
            <button onClick={onCopy} className="rounded-xl bg-slate-900 px-3 py-1.5 text-[10px] font-extrabold text-white transition-colors hover:bg-slate-800">{copied ? "Copied" : "Copy"}</button>
          </div>
        </div>
      </div>
    </div>
